const spawnLocations = [
  {
    spawnLocation: "Swamp Trolls Port Hope/Venore",
    recommendedLevel: "8+",
    expectedRawXp: "25k/h",
    expectedLoot: "50k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Larva Ankh",
    recommendedLevel: "8+",
    expectedRawXp: "25k/h",
    expectedLoot: "5k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Rotworms Liberty Bay/Darashia",
    recommendedLevel: "8+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Crocodiles Port Hope",
    recommendedLevel: "8+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Edron Forgotten Tomb",
    recommendedLevel: "14+",
    expectedRawXp: "60k/h",
    expectedLoot: "80k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Tarantulas Port Hope",
    recommendedLevel: "14+",
    expectedRawXp: "50k/h",
    expectedLoot: "15k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Larva/Mummy MOSL -1/-2",
    recommendedLevel: "14+",
    expectedRawXp: "40k/h",
    expectedLoot: "50k/h",
    linkToVideo: "https://youtu.be/hpCXYpMPudM"
  },
  {
    spawnLocation: "Amazon Camp (Profit)",
    recommendedLevel: "14+",
    expectedRawXp: "30k/h",
    expectedLoot: "60k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Minotaurs Darashia (Stealth Ring)",
    recommendedLevel: "15+",
    expectedRawXp: "80k/h",
    expectedLoot: "-20k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Elves Yalahar",
    recommendedLevel: "15+",
    expectedRawXp: "50k/h",
    expectedLoot: "100k/h",
    linkToVideo: "https://youtu.be/ava03fFNOzs"
  },
  {
    spawnLocation: "Cyclops Mistrock/Mount Sternum",
    recommendedLevel: "15+",
    expectedRawXp: "50k/h",
    expectedLoot: "-20k/h",
    linkToVideo: "https://youtu.be/YEl_uzXqAFc"
  },
  {
    spawnLocation: "Stonerefiners (Stealth Ring)",
    recommendedLevel: "20+",
    expectedRawXp: "140k/h",
    expectedLoot: "25k/h",
    linkToVideo: "https://youtu.be/_ILuesC-eK0"
  },
  {
    spawnLocation: "Mutated Humans Alchemist",
    recommendedLevel: "20+",
    expectedRawXp: "80k/h",
    expectedLoot: "15k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Chor (Runes)",
    recommendedLevel: "30+",
    expectedRawXp: "230k/h",
    expectedLoot: "-25k/h",
    linkToVideo: "https://youtu.be/51DLTRgGlOM"
  },
  {
    spawnLocation: "Banuta Apes (Runes)",
    recommendedLevel: "30+",
    expectedRawXp: "110k/h",
    expectedLoot: "100k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Carniphilas Port Hope",
    recommendedLevel: "30+",
    expectedRawXp: "90k/h",
    expectedLoot: "10k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Coryms PH (Runes)",
    recommendedLevel: "35+",
    expectedRawXp: "250k/h",
    expectedLoot: "-80k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Upper Spike (Runes)",
    recommendedLevel: "35+",
    expectedRawXp: "250k/h",
    expectedLoot: "10k/h",
    linkToVideo: "https://youtu.be/Sad4UxhUrlc"
  },
  {
    spawnLocation: "MoSL -3",
    recommendedLevel: "35+",
    expectedRawXp: "200k/h",
    expectedLoot: "180k/h",
    linkToVideo: "https://youtu.be/ITT8E3gg2aA"
  },
  {
    spawnLocation: "Peninsula Tomb",
    recommendedLevel: "35+",
    expectedRawXp: "185k/h",
    expectedLoot: "90k/h",
    linkToVideo: "https://youtu.be/zdm2xuJhc7A"
  },
  {
    spawnLocation: "Mutated Rats Yalahar Alchemist",
    recommendedLevel: "35+",
    expectedRawXp: "180k/h",
    expectedLoot: "-40k/h",
    linkToVideo: "https://youtu.be/AjAHNwB1B_4"
  },
  {
    spawnLocation: "Meriana Gargoyle Cave (Runes)",
    recommendedLevel: "35+",
    expectedRawXp: "160k/h",
    expectedLoot: "10k/h",
    linkToVideo: "https://youtu.be/Ryu3py8afk4"
  },
  {
    spawnLocation: "Laguna Blood Crabs (Runes)",
    recommendedLevel: "35+",
    expectedRawXp: "160k/h",
    expectedLoot: "350k/h",
    linkToVideo: "https://youtu.be/ru3lLIUJ8eI"
  },
  {
    spawnLocation: "Mutated Rats Yalahar Arena",
    recommendedLevel: "35+",
    expectedRawXp: "150k/h",
    expectedLoot: "-30k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Yalahar Cults",
    recommendedLevel: "40+",
    expectedRawXp: "180k/h",
    expectedLoot: "250k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "LB Quara Scouts",
    recommendedLevel: "40+",
    expectedRawXp: "180k/h",
    expectedLoot: "60k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Wyrms LB",
    recommendedLevel: "50+",
    expectedRawXp: "420k/h",
    expectedLoot: "-50k/h",
    linkToVideo: "https://youtu.be/DujCnEzUF4A"
  },
  {
    spawnLocation: "Lion's Rock",
    recommendedLevel: "50+",
    expectedRawXp: "375k/h",
    expectedLoot: "0k/h",
    linkToVideo: "https://youtu.be/ADda5KzhdG0"
  },
  {
    spawnLocation: "Edron Bonebeasts",
    recommendedLevel: "50+",
    expectedRawXp: "375k/h",
    expectedLoot: "-50k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Killer Caiman Cave Zao",
    recommendedLevel: "50+",
    expectedRawXp: "320k/h",
    expectedLoot: "-20k/h",
    linkToVideo: "https://youtu.be/5Oez2-DKo40"
  },
  {
    spawnLocation: "Vengoth Haunted Treelings",
    recommendedLevel: "50+",
    expectedRawXp: "320k/h",
    expectedLoot: "-80k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Kha'Zeel Ancient Scarabs",
    recommendedLevel: "50+",
    expectedRawXp: "300k/h",
    expectedLoot: "0k/h",
    linkToVideo: "https://youtu.be/xKoWjt3bG74"
  },
  {
    spawnLocation: "Water Elementals South Port Hope",
    recommendedLevel: "60+",
    expectedRawXp: "650k/h",
    expectedLoot: "80k/h",
    linkToVideo: "https://youtu.be/1muhbsZofTM"
  },
  {
    spawnLocation: "Water Elementals West Port Hope",
    recommendedLevel: "60+",
    expectedRawXp: "440k/h",
    expectedLoot: "20k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Nibelor Crystal Spiders",
    recommendedLevel: "60+",
    expectedRawXp: "440k/h",
    expectedLoot: "-30k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Nightmare Scions Krailos",
    recommendedLevel: "70+",
    expectedRawXp: "900k/h",
    expectedLoot: "-150k/h",
    linkToVideo: "https://youtu.be/5HBGq2LeyYQ"
  },
  {
    spawnLocation: "Quaras LB",
    recommendedLevel: "70+",
    expectedRawXp: "850k/h",
    expectedLoot: "-40k/h",
    linkToVideo: "https://youtu.be/npeHnaD7T78"
  },
  {
    spawnLocation: "Edron Vampire Crypts",
    recommendedLevel: "70+",
    expectedRawXp: "750k/h",
    expectedLoot: "300k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Iksupan Entrance",
    recommendedLevel: "70+",
    expectedRawXp: "750k/h",
    expectedLoot: "200k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Mother of Scarabs Lair",
    recommendedLevel: "70+",
    expectedRawXp: "750k/h",
    expectedLoot: "-50k/h",
    linkToVideo: "https://youtu.be/B3BPkDNBOck"
  },
  {
    spawnLocation: "Yalahar Bog Raiders Central",
    recommendedLevel: "70+",
    expectedRawXp: "600k/h",
    expectedLoot: "-50k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Fenrock Dragon Lords",
    recommendedLevel: "75+",
    expectedRawXp: "950k/h",
    expectedLoot: "-100k/h",
    linkToVideo: "https://youtu.be/PxwzDs_xEjM"
  },
  {
    spawnLocation: "POI Dragon Lords",
    recommendedLevel: "75+",
    expectedRawXp: "900k/h",
    expectedLoot: "-100k/h",
    linkToVideo: "https://youtu.be/1Ff_F8ZCOno"
  },
  {
    spawnLocation: "Middle Spike",
    recommendedLevel: "75+",
    expectedRawXp: "850k/h",
    expectedLoot: "-30k/h",
    linkToVideo: "https://youtu.be/SrP08k3tUyE"
  },
  {
    spawnLocation: "Edron Old Fortress Heroes",
    recommendedLevel: "80+",
    expectedRawXp: "1.4kk/h",
    expectedLoot: "80k/h",
    linkToVideo: "https://youtu.be/sBV8GR3K-rk"
  },
  {
    spawnLocation: "Carlin Cults",
    recommendedLevel: "80+",
    expectedRawXp: "1.3kk/h",
    expectedLoot: "100k/h",
    linkToVideo: "https://youtu.be/Llb61wMXBP4"
  },
  {
    spawnLocation: "Inner Hive Stage2 (East)",
    recommendedLevel: "80+",
    expectedRawXp: "900k/h",
    expectedLoot: "300k/h",
    linkToVideo: "https://youtu.be/T4X_MoLPa8o"
  },
  {
    spawnLocation: "Yielothaxes",
    recommendedLevel: "80+",
    expectedRawXp: "900k/h",
    expectedLoot: "250k/h",
    linkToVideo: "https://youtu.be/QUZCbA0c5Eg"
  },
  {
    spawnLocation: "Giant Spiders Task - Port Hope",
    recommendedLevel: "80+",
    expectedRawXp: "600k/h",
    expectedLoot: "-50k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Sunken Quarter",
    recommendedLevel: "90+",
    expectedRawXp: "1.4kk/h",
    expectedLoot: "150k/h",
    linkToVideo: "https://youtu.be/xD_W5_GCFFE"
  },
  {
    spawnLocation: "Okolnir",
    recommendedLevel: "90+",
    expectedRawXp: "1.25kk/h",
    expectedLoot: "-80k/h",
    linkToVideo: "https://youtu.be/oKgNT3cTnaw"
  },
  {
    spawnLocation: "Orclops",
    recommendedLevel: "90+",
    expectedRawXp: "1kk/h",
    expectedLoot: "150k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Dark Faun Cave",
    recommendedLevel: "90+",
    expectedRawXp: "1kk/h",
    expectedLoot: "100k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Souleater Mountain",
    recommendedLevel: "90+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Oramond West",
    recommendedLevel: "90+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Hydras Oramond",
    recommendedLevel: "90+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Putrid Mummy",
    recommendedLevel: "90+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Werehyaenas -1",
    recommendedLevel: "90+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Ravenous Lava Lurkers",
    recommendedLevel: "100+",
    expectedRawXp: "2.1kk/h",
    expectedLoot: "-50k/h",
    linkToVideo: "https://youtu.be/Pom_8ybPnZw"
  },
  {
    spawnLocation: "Lizard Chosens",
    recommendedLevel: "100+",
    expectedRawXp: "1.25kk/h",
    expectedLoot: "80k/h",
    linkToVideo: "https://youtu.be/7LaPIGEF4VM"
  },
  {
    spawnLocation: "Mini Rosha",
    recommendedLevel: "100+",
    expectedRawXp: "1.15kk/h",
    expectedLoot: "450k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Replica Dungeon",
    recommendedLevel: "100+",
    expectedRawXp: "1.1kk/h",
    expectedLoot: "-50k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Edron Orc Cults",
    recommendedLevel: "100+",
    expectedRawXp: "1.0kk/h",
    expectedLoot: "250k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Exotic Cave",
    recommendedLevel: "100+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Barkless Elves Ab",
    recommendedLevel: "130+",
    expectedRawXp: "2.2kk/h",
    expectedLoot: "-400k/h",
    linkToVideo: "https://youtu.be/YXDva_Orwgs"
  },
  {
    spawnLocation: "Elder Wyrms Drefia",
    recommendedLevel: "130+",
    expectedRawXp: "1.45kk/h",
    expectedLoot: "120k/h",
    linkToVideo: "https://youtu.be/pv5Ro46N1qk"
  },
  {
    spawnLocation: "Brimstone Bugs WOTE",
    recommendedLevel: "130+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Deeplings Library",
    recommendedLevel: "130+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Glooth Bandits",
    recommendedLevel: "130+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Otherworld (Kazo)",
    recommendedLevel: "150+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Yalahar Grim Reapers",
    recommendedLevel: "150+",
    expectedRawXp: "3.3kk/h",
    expectedLoot: "-150k/h",
    linkToVideo: "https://youtu.be/upayVKk-XYM"
  },
  {
    spawnLocation: "Deeper Banuta -6",
    recommendedLevel: "150+",
    expectedRawXp: "1.9kk/h",
    expectedLoot: "400k/h",
    linkToVideo: "https://youtu.be/bP3pGqPJXCA"
  },
  {
    spawnLocation: "Glooth Tower",
    recommendedLevel: "150+",
    expectedRawXp: "1.9kk/h",
    expectedLoot: "200k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Wildlife Raid (Active)",
    recommendedLevel: "200+",
    expectedRawXp: "2.8kk/h",
    expectedLoot: "600k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Werehyaenas",
    recommendedLevel: "200+",
    expectedRawXp: "2.5kk/h",
    expectedLoot: "550k/h",
    linkToVideo: "https://youtu.be/aunNgm5-XsI"
  },
  {
    spawnLocation: "Medusa Tower",
    recommendedLevel: "200+",
    expectedRawXp: "2.4kk/h",
    expectedLoot: "400k/h",
    linkToVideo: "https://youtu.be/iUp7dma4WuI"
  },
  {
    spawnLocation: "Seacrest Serpents",
    recommendedLevel: "200+",
    expectedRawXp: "2.4kk/h",
    expectedLoot: "500k/h",
    linkToVideo: "https://youtu.be/MOoFHfbgltY"
  },
  {
    spawnLocation: "Draken Walls",
    recommendedLevel: "200+",
    expectedRawXp: "2.1kk/h",
    expectedLoot: "450k/h",
    linkToVideo: "https://youtu.be/X2gGK2LshUQ"
  },
  {
    spawnLocation: "Otherworld (Edron)",
    recommendedLevel: "200+",
    expectedRawXp: "2.1kk/h",
    expectedLoot: "250k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Hive Stage3",
    recommendedLevel: "200+",
    expectedRawXp: "2.1kk/h",
    expectedLoot: "600k/h",
    linkToVideo: "https://youtu.be/vtzHQA05xC4"
  },
  {
    spawnLocation: "Book World: Chapter I/II/III/IV",
    recommendedLevel: "200+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Asura Palace",
    recommendedLevel: "250+",
    expectedRawXp: "2.9kk/h",
    expectedLoot: "400k/h",
    linkToVideo: "https://youtu.be/exfl57YQCAA"
  },
  {
    spawnLocation: "Candia Nibblemaws",
    recommendedLevel: "250+",
    expectedRawXp: "3.1kk/h",
    expectedLoot: "600k/h",
    linkToVideo: "https://youtu.be/9505L-ZXtxU"
  },
  {
    spawnLocation: "Lower Spike",
    recommendedLevel: "250+",
    expectedRawXp: "2.5kk/h",
    expectedLoot: "200k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Deeper Banuta -8",
    recommendedLevel: "250+",
    expectedRawXp: "2.5kk/h",
    expectedLoot: "450k/h",
    linkToVideo: "https://youtu.be/i9ecWlmgObE"
  },
  {
    spawnLocation: "Carnivoras",
    recommendedLevel: "250+",
    expectedRawXp: "2.0kk/h",
    expectedLoot: "1kk/h",
    linkToVideo: "https://youtu.be/0mXEw6BijN4"
  },
  {
    spawnLocation: "Pirats",
    recommendedLevel: "250+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Asura Mirror",
    recommendedLevel: "270+",
    expectedRawXp: "5.2kk/h",
    expectedLoot: "500k/h",
    linkToVideo: "https://youtu.be/UyZPXrfogSA"
  },
  {
    spawnLocation: "Warzone 5",
    recommendedLevel: "270+",
    expectedRawXp: "3.8kk/h",
    expectedLoot: "1kk/h",
    linkToVideo: "https://youtu.be/jRNXHzlcAOc"
  },
  {
    spawnLocation: "Oramond Fury Dungeon",
    recommendedLevel: "270+",
    expectedRawXp: "3.8kk/h",
    expectedLoot: "900k/h",
    linkToVideo: "https://youtu.be/N_bKqXT3Xco"
  },
  {
    spawnLocation: "Werelions",
    recommendedLevel: "270+",
    expectedRawXp: "3.0kk/h",
    expectedLoot: "900k/h",
    linkToVideo: "https://youtu.be/TcyeTVUn_Lg"
  },
  {
    spawnLocation: "Marapur Nagas",
    recommendedLevel: "300+",
    expectedRawXp: "5.3kk/h",
    expectedLoot: "400k/h",
    linkToVideo: "https://youtu.be/JErJMHqu9dc"
  },
  {
    spawnLocation: "Roshamuul Mines",
    recommendedLevel: "300+",
    expectedRawXp: "4.8kk/h",
    expectedLoot: "750k/h",
    linkToVideo: "https://youtu.be/PiIH9VhMiEc"
  },
  {
    spawnLocation: "Winter Court",
    recommendedLevel: "300+",
    expectedRawXp: "4.6kk/h",
    expectedLoot: "750k/h",
    linkToVideo: "https://youtu.be/lky2cG7w8j8"
  },
  {
    spawnLocation: "Summer Court",
    recommendedLevel: "300+",
    expectedRawXp: "4.5kk/h",
    expectedLoot: "650k/h",
    linkToVideo: "https://youtu.be/DTabKIwv04E"
  },
  {
    spawnLocation: "Oskayaat Weretigers",
    recommendedLevel: "300+",
    expectedRawXp: "4.1kk/h",
    expectedLoot: "700k/h",
    linkToVideo: "https://youtu.be/0TKdpEhssk0"
  },
  {
    spawnLocation: "Burster Spectres",
    recommendedLevel: "300+",
    expectedRawXp: "4.0kk/h",
    expectedLoot: "200k/h",
    linkToVideo: "https://youtu.be/ClbgZRc7_Po"
  },
  {
    spawnLocation: "Oskayaat Werecrocodiles",
    recommendedLevel: "300+",
    expectedRawXp: "3.6kk/h",
    expectedLoot: "750k/h",
    linkToVideo: "https://youtu.be/gFjDuUjVUdE"
  },
  {
    spawnLocation: "Warzone 4",
    recommendedLevel: "300+",
    expectedRawXp: "3.3kk/h",
    expectedLoot: "750k/h",
    linkToVideo: "https://youtu.be/MQxUHFAYPOM"
  },
  {
    spawnLocation: "Warzone 6",
    recommendedLevel: "300+",
    expectedRawXp: "3.2kk/h",
    expectedLoot: "1.0kk/h",
    linkToVideo: "https://youtu.be/3KuBpC4i8DU"
  },
  {
    spawnLocation: "Lower Rosh",
    recommendedLevel: "300+",
    expectedRawXp: "3.1kk/h",
    expectedLoot: "600k/h",
    linkToVideo: "https://youtu.be/QI49YMtE7go"
  },
  {
    spawnLocation: "Gazer Spectre",
    recommendedLevel: "300+",
    expectedRawXp: "3.0kk/h",
    expectedLoot: "600k/h",
    linkToVideo: "https://youtu.be/JpKuYWJnjdc"
  },
  {
    spawnLocation: "Marapur Turtles",
    recommendedLevel: "300+",
    expectedRawXp: "2.6kk/h",
    expectedLoot: "1.15kk/h",
    linkToVideo: "https://youtu.be/7ruc0mkSfI4"
  },
  {
    spawnLocation: "Otherworld",
    recommendedLevel: "300+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Iksupan Occupied Sanctuary",
    recommendedLevel: "350+",
    expectedRawXp: "5.1kk/h",
    expectedLoot: "750k/h",
    linkToVideo: "https://youtu.be/KdSJTasIm0U"
  },
  {
    spawnLocation: "Buried Cathedral",
    recommendedLevel: "350+",
    expectedRawXp: "4.9kk/h",
    expectedLoot: "300k/h",
    linkToVideo: "https://youtu.be/KrVEDJUBnVA"
  },
  {
    spawnLocation: "Iksupan Last Stand (Entrance)",
    recommendedLevel: "350+",
    expectedRawXp: "4.3kk/h",
    expectedLoot: "700k/h",
    linkToVideo: "https://youtu.be/2QHcDnyOU7c"
  },
  {
    spawnLocation: "Marapur Nagas (Soulkamas)",
    recommendedLevel: "400+",
    expectedRawXp: "6.6kk/h",
    expectedLoot: "750k/h",
    linkToVideo: "https://youtu.be/JErJMHqu9dc"
  },
  {
    spawnLocation: "Cobra Bastion -1 (Basement)",
    recommendedLevel: "400+",
    expectedRawXp: "6.3kk/h",
    expectedLoot: "700k/h",
    linkToVideo: "https://youtu.be/r1l04SEiFpo"
  },
  {
    spawnLocation: "Crypt Warriors",
    recommendedLevel: "400+",
    expectedRawXp: "5.9kk/h",
    expectedLoot: "700k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Upper Roshamuul",
    recommendedLevel: "400+",
    expectedRawXp: "5.8kk/h",
    expectedLoot: "1.15kk/h",
    linkToVideo: "https://youtu.be/gCNQ8G6hI7E"
  },
  {
    spawnLocation: "Falcons",
    recommendedLevel: "400+",
    expectedRawXp: "5.6kk/h",
    expectedLoot: "1.3kk/h",
    linkToVideo: "https://youtu.be/UBCXn-gEA6o"
  },
  {
    spawnLocation: "Deathlings",
    recommendedLevel: "400+",
    expectedRawXp: "5.3kk/h",
    expectedLoot: "800k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Azzilon Walls",
    recommendedLevel: "400+",
    expectedRawXp: "5.3kk/h",
    expectedLoot: "500k/h",
    linkToVideo: "https://youtu.be/J_9p5NWlcPU"
  },
  {
    spawnLocation: "Sphinx/Crypt Warden Issavi",
    recommendedLevel: "400+",
    expectedRawXp: "5.2kk/h",
    expectedLoot: "700k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Rosha West",
    recommendedLevel: "400+",
    expectedRawXp: "4.8kk/h",
    expectedLoot: "1.1kk/h",
    linkToVideo: "https://youtu.be/t6iReSRaWfA"
  },
  {
    spawnLocation: "Oramond Catacombs",
    recommendedLevel: "400+",
    expectedRawXp: "4.7kk/h",
    expectedLoot: "800k/h",
    linkToVideo: "https://youtu.be/uf4aFM9IxeY"
  },
  {
    spawnLocation: "Warzone 3",
    recommendedLevel: "400+",
    expectedRawXp: "4.7kk/h",
    expectedLoot: "800k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Floating Savants MoTA",
    recommendedLevel: "400+",
    expectedRawXp: "4.0kk/h",
    expectedLoot: "1.0kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Oramond Hideout -1",
    recommendedLevel: "400+",
    expectedRawXp: "3.8kk/h",
    expectedLoot: "1.0kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Azzilon Castle",
    recommendedLevel: "450+",
    expectedRawXp: "6.3kk/h",
    expectedLoot: "600k/h",
    linkToVideo: "https://youtu.be/w4gaxOlvFf8"
  },
  {
    spawnLocation: "Flimsy Port Hope",
    recommendedLevel: "450+",
    expectedRawXp: "5.1kk/h",
    expectedLoot: "350k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Cobra Bastion",
    recommendedLevel: "500+",
    expectedRawXp: "7.0kk/h",
    expectedLoot: "700k/h",
    linkToVideo: "https://youtu.be/cOtyvzz1Htk"
  },
  {
    spawnLocation: "Issavi Goannas - Central Steppe",
    recommendedLevel: "500+",
    expectedRawXp: "5.6kk/h",
    expectedLoot: "500k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Plague Feru Seal",
    recommendedLevel: "500+",
    expectedRawXp: "5.4kk/h",
    expectedLoot: "1.4kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Pumin Feru Seal",
    recommendedLevel: "500+",
    expectedRawXp: "5.0kk/h",
    expectedLoot: "1.5kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Girtablilu",
    recommendedLevel: "500+",
    expectedRawXp: "4.2kk/h",
    expectedLoot: "1.6kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Bashmu",
    recommendedLevel: "500+",
    expectedRawXp: "3.5kk/h",
    expectedLoot: "1.8kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Warzone 7",
    recommendedLevel: "500+",
    expectedRawXp: "3.5kk/h",
    expectedLoot: "1.5kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Norcferatu Castle",
    recommendedLevel: "500+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "True Asuras",
    recommendedLevel: "600+",
    expectedRawXp: "5.5kk/h",
    expectedLoot: "1.3kk/h",
    linkToVideo: "https://youtu.be/M7-OckvO6hY"
  },
  {
    spawnLocation: "Undead Dragon Feru Seal",
    recommendedLevel: "600+",
    expectedRawXp: "5.6kk/h",
    expectedLoot: "1.2kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Infernatil Feru Seal",
    recommendedLevel: "600+",
    expectedRawXp: "5.3kk/h",
    expectedLoot: "1kk/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Juggernaut Feru Seal",
    recommendedLevel: "600+",
    expectedRawXp: "5.0kk/h",
    expectedLoot: "800k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Norcferatu Dungeons West",
    recommendedLevel: "600+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Norcferatu Dungeons East",
    recommendedLevel: "600+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Issavi Sewers",
    recommendedLevel: "600+",
    expectedRawXp: "5.0kk/h",
    expectedLoot: "900k/h",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Otherworld (Sanguine Claws)",
    recommendedLevel: "600+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Warzone 8",
    recommendedLevel: "600+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Wardragons",
    recommendedLevel: "600+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Darashia Skeletons",
    recommendedLevel: "600+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  },
  {
    spawnLocation: "Warzone 9",
    recommendedLevel: "800+",
    expectedRawXp: "Unknown",
    expectedLoot: "Unknown",
    linkToVideo: "No video available"
  }
];
export default spawnLocations;